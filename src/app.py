...
if (st.session_state["tle_file_obj"] is not None):
    ...
    st.error("No TLE file provided! Use the side bar to upload a TLE or select option to use the latest TLE from Celestrack")
    st.stop()

def write_doppler_data_to_csv(doppler_shifts, all_graph, selected_satellites=None, filename="doppler_data.csv"):
    """
    Create CSV data with Doppler shifts, azimuth, elevation, and distance.
    """
    rows = []
    
    # Filter satellites if selection is provided
    sats_to_export = selected_satellites if selected_satellites is not None else doppler_shifts.keys()
    
    for sat in sats_to_export:
        if sat in doppler_shifts and sat in all_graph:
            shifts = doppler_shifts[sat]
            pos_data = all_graph[sat]
            
            # Create a lookup dictionary for position data by timestamp
            pos_lookup = {}
            for ts, alt, azimuth, distance in pos_data:
                pos_lookup[ts] = (azimuth.degrees, alt.degrees, distance.km)
            
            for timestamp, doppler_shift in shifts:
                # Get position data from lookup
                az, el, dist = pos_lookup.get(timestamp, (None, None, None))
                
                rows.append({
                    'Satellite': sat,
                    'Timestamp (UTC)': timestamp,
                    'Doppler Shift (Hz)': float(doppler_shift),
                    'Azimuth (deg)': az if az is not None else '',
                    'Elevation (deg)': el if el is not None else '',
                    'Distance (km)': dist if dist is not None else ''
                })
...
def write_fov_satellites_to_csv(visible_satellites, selected_satellites=None, filename="fov_satellites.csv"):
    """
    Create CSV data with satellite names and event counts.
    """
    rows = []
    
    # Filter satellites if selection is provided
    sats_to_export = selected_satellites if selected_satellites is not None else visible_satellites.keys()
    
    for sat in sats_to_export:
        if sat in visible_satellites:
            sat_info = visible_satellites[sat]
            
            # Count events for each satellite
            rise_count = len(sat_info.get(0, []))  # Rise events
            peak_count = len(sat_info.get(1, []))  # Peak events  
            set_count = len(sat_info.get(2, []))   # Set events
            
            # Only include satellites with at least one event
            if rise_count > 0 or peak_count > 0 or set_count > 0:
                rows.append({
                    'Satellite': sat,
                    'Rise Events': rise_count,
                    'Peak Events': peak_count,
                    'Set Events': set_count
                })
...
with tab1:
    ...
    with col1:
        if st.button("Export Doppler Data to CSV"):
            # Create dynamic filename based on time window
            start_str = start_datetime.strftime("%Y%m%d_%H%M")
            end_str = end_datetime.strftime("%Y%m%d_%H%M")
            filename = f"doppler_data_{start_str}_to_{end_str}.csv"
            
            result = write_doppler_data_to_csv(doppler_shifts, all_graph, filename=filename)
            if result:
    ...
    with col2:
        if st.button("Export Fov Data to CSV"):
            # Create dynamic filename based on time window
            start_str = start_datetime.strftime("%Y%m%d_%H%M")
            end_str = end_datetime.strftime("%Y%m%d_%H%M")
            filename = f"fov_satellites_{start_str}_to_{end_str}.csv"
            
            result = write_fov_satellites_to_csv(visible_satellites, filename=filename)
            if result:
                csv_data, filename = result
                st.download_button(
                    label="Download CSV",
